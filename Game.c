...
void Visible_update(Point topLeft, unsigned * leftIndex, unsigned * rightIndex){
    ...
    *rightIndex = (topLeft.x+SCREEN_X-1)/MAP_STEP;
}
...
void update_objects(unsigned leftIndex, unsigned rightIndex){
    GAME.Zero.x += GAME.Zero.dx;
    GAME.Zero.y += GAME.Zero.dy;
}

void Main_move (Point * topLeft){
    topLeft->x = GAME.Zero.x - SCREEN_X/2+1 + (GAME.spritesArr[OBJECT_ID_ZERO].b+1)/2;
}